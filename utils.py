...
from gradvac_imtl import PCGrad
...
def multi_task_trainer(train_loader, test_loader, multi_task_model, device, optimizer, scheduler, opt, total_epoch=200):
    ...
    T = opt.temp        # '--temp', default=2.0
    avg_cost = np.zeros([total_epoch, 12], dtype=np.float32)        # total_epoch=200
    lambda_weight = np.ones([2, total_epoch])
    for index in range(total_epoch):
        cost = np.zeros(12, dtype=np.float32)

        # apply Dynamic Weight Average
        if opt.weight == 'dwa':        # '--weight', default='equal'
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                # test_data, test_label, test_depth = test_dataset.next()        # torch-1.8.1
                test_data, test_label, test_depth = next(test_dataset)        # torch-2.3.0
        ...
        print('Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f}|'
            ...
            .format(index, avg_cost[index, 0], avg_cost[index, 1], avg_cost[index, 2], avg_cost[index, 3],
                    ...
                    # , avg_cost[index, 12], avg_cost[index, 13],
...
"""
def multi_task_trainer(train_loader, test_loader, multi_task_model, device, optimize, scheduler, opt, total_epoch=200):
    train_batch = len(train_loader)
    test_batch = len(test_loader)
    # T = opt.temp        # '--temp', default=2.0        # dwa 算法需要用到
    avg_cost = np.zeros([total_epoch, 12], dtype=np.float32)  # total_epoch=200
    # lambda_weight = np.ones([2, total_epoch])        # dwa 算法需要用到
    for index in range(total_epoch):
        cost = np.zeros(12, dtype=np.float32)
        ...
        for k in range(train_batch):
            ...
            '''
            # loss.backward()
            losses = [train_loss[0], train_loss[1]]  # a list of per-task losses
    ...
    for i in range(500):
        path0 = './output1/seg'
        path1 = './output1/depth'
        savepath0 = os.path.join(path0, '{}.npy'.format(i))
        savepath1 = os.path.join(path1, '{}.npy'.format(i))
        test_pred0 = test_pred[0].cpu().numpy()
        test_pred1 = test_pred[1].cpu().numpy()
        np.save(savepath0, test_pred0)
        np.save(savepath1, test_pred1)
"""


def multi_task_trainer_qt(train_loader, test_loader, multi_task_model, device, optimizer, scheduler, opt,
                          total_epoch=200):
    method = opt.method
    alpha = 0.5
    ...
    grads = torch.Tensor(sum(grad_dims), 2)
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            # if opt.weight == 'dirichlet':
            #   probs = np.sum(np.random.dirichlet(np.ones(2), size=2), axis=0)/2
            #  for i in range(2):
            #    train_loss_tmp[i] = train_loss[i] * probs[i]
            optimizer.zero_grad()
