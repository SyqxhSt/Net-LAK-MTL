...
class KVMSANet(nn.Module):
    def __init__(self):
        ...
        for j in range(2):  # j 表示第几个任务
            ...
            for i in range(2):  # i 表示第 j 个任务的，第几个注意力模块
    ...
    def forward(self, x):
        g_encoder, g_decoder, g_maxpool, g_upsampl, indices = ([0] * 3 for _ in range(5))        # range(5) 中的 5 取决于前面要赋值给 5 个变量
        for i in range(3):
        ...
        for i in range(2):
            atten_encoder[i], atten_decoder[i] = ([0] * 3 for _ in range(2))
        for i in range(2):
            for j in range(3):
