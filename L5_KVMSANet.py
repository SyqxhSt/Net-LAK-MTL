...
class KVMSANet(nn.Module):
    def __init__(self):
        ...
        # 专门适应 KVMSANet（不含 KConv 解码器（非对称解码器）） 中通道数
        KConv_dec_channel = [64, 128, 256, 512, 512, 512]
        
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

        self.class_nb = 7        # Cityscapes ——> 7    KITTI ——> 14    Nyuv2 ——> 13

        # define encoder decoder layers
        self.encoder_block = nn.ModuleList([self.conv_layer([3, filter[0]])])
        # self.decoder_block = nn.ModuleList([self.conv_layer([filter[0], filter[0]])])        # 原 VMSANet 解码器部分
        for i in range(4):
            self.encoder_block.append(self.conv_layer([filter[i], filter[i + 1]]))
            # self.decoder_block.append(self.conv_layer([filter[i + 1], filter[i]]))        # 原 VMSANet 解码器部分
            
        
        # SE模块
        self.encoder_SE_block = nn.ModuleList([SEModule(filter[0])])
        # self.decoder_SE_block = nn.ModuleList([SEModule(dechannel[0])])        # 原 VMSANet 解码器部分
        for i in range(4):
            self.encoder_SE_block.append(SEModule(filter[i + 1]))
            # self.decoder_SE_block.append(SEModule(dechannel[i + 1]))        # 原 VMSANet 解码器部分
        

        '''
        # PAFE模块
        self.encoder_PAFE_block = nn.ModuleList([PAFEM(PAFE_en_channel[0], PAFE_en_channel[1])])
        # self.decoder_PAFE_block = nn.ModuleList([PAFEM(PAFE_de_channel[4], PAFE_de_channel[5])])        # 原 VMSANet 解码器部分
        for i in range(4):
            self.encoder_PAFE_block.append(PAFEM(PAFE_en_channel[i + 1], PAFE_en_channel[i + 2]))
            # self.decoder_PAFE_block.append(PAFEM(PAFE_de_channel[-i - 3], PAFE_de_channel[-i - 2]))        # 原 VMSANet 解码器部分
        '''
        
        # define convolution layer
        self.conv_block_enc = nn.ModuleList([self.conv_layer([filter[0], filter[0]])])
        # self.conv_block_dec = nn.ModuleList([self.conv_layer([filter[0], filter[0]])])        # 原 VMSANet 解码器部分
        for i in range(4):
            if i == 0:
                self.conv_block_enc.append(self.conv_layer([filter[i + 1], filter[i + 1]]))
                # self.conv_block_dec.append(self.conv_layer([filter[i], filter[i]]))        # 原 VMSANet 解码器部分
            else:
        ...
        # 模型开头与最后输出用 Kanconv 时的代码
        self.kanconv_input = KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device)
        
        # self.kanconv_task1 = KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device)
        # self.kanconv_task2 = KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device)
        ...
        self.conv_block_enc.append(KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device))
        self.conv_block_enc.append(KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device))
        self.conv_block_enc.append(KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device))
        self.conv_block_enc.append(KAN_Convolutional_Layer(n_convs = 1,padding = (1,1),kernel_size= (3,3),device = device))
        '''
        ...
        self.encoder_att = nn.ModuleList([nn.ModuleList([self.att_layer([filter[0], filter[0], filter[0]])])])        # 双 nn.ModuleList 嵌套才有最前面的维度
        # self.decoder_att = nn.ModuleList([nn.ModuleList([self.att_layer([2 * filter[0], filter[0], filter[0]])])])        # 原 VMSANet 解码器部分
        self.encoder_block_att = nn.ModuleList([self.conv_layer([filter[0], filter[1]])])
        # self.decoder_block_att = nn.ModuleList([self.conv_layer([filter[0], filter[0]])])        # 原 VMSANet 解码器部分
        ...
        for j in range(2):  # j 表示第几个任务
            if j < 1:
                ...
                # self.decoder_att.append(nn.ModuleList([self.att_layer([2 * filter[0], filter[0], filter[0]])]))        # 原 VMSANet 解码器部分
            for i in range(4):  # i 表示第 j 个任务的，第几个注意力模块
                self.encoder_att[j].append(self.att_layer([2 * filter[i + 1], filter[i + 1], filter[i + 1]]))
                # self.decoder_att[j].append(self.att_layer([filter[i + 1] + filter[i], filter[i], filter[i]]))        # 原 VMSANet 解码器部分

        for i in range(4):
            if i < 3:
                self.encoder_block_att.append(self.conv_layer([filter[i + 1], filter[i + 2]]))
                # self.decoder_block_att.append(self.conv_layer([filter[i + 1], filter[i]]))        # 原 VMSANet 解码器部分
            else:
                self.encoder_block_att.append(self.conv_layer([filter[i + 1], filter[i + 1]]))
                # self.decoder_block_att.append(self.conv_layer([filter[i + 1], filter[i + 1]]))        # 原 VMSANet 解码器部分

        # KVMSANet（非对称解码器部分）
        self.decoder_block = nn.ModuleList([nn.ModuleList([self.conv_layer([512, 512])])])        # 创建了 decoder_block[0][0]
        for j in range(2):        # j 表示一共几个任务
            if j < 1:
                self.decoder_block.append(nn.ModuleList([self.conv_layer([512, 512])]))        # 创建了 decoder_block[1][0]
            for i in range(4):
                self.decoder_block[j].append(self.conv_layer([KConv_dec_channel[-i - 2], KConv_dec_channel[-i - 3]]))
                
        # KVMSANet（非对称解码器最后输出部分）
        self.decoder_output = nn.ModuleList([self.conv_layer([KConv_dec_channel[0], KConv_dec_channel[0]])])
        self.decoder_output.append(self.conv_layer([KConv_dec_channel[0], KConv_dec_channel[0]]))
    ...
    def forward(self, x):
        ...
        # x = self.kanconv_input(x)
        
        # define global shared network
        for i in range(5):  # 编码器部分 (下采样)
            if i == 0:
        ...
        '''
        # 原 VMSANet 解码部分（无 KConv）
        for i in range(5):  # 解码器部分 (上采样)
            if i == 0:
        ...
        for i in range(2):  # i 表任务数量
            ...
            for j in range(5):
                if j == 0:
                    atten_decoder[i][j][0] = self.decoder_block[i][j](atten_encoder[i][-1][-1])
                    # print(f'解码器中，经过decoder_block后，atten_decoder[{i}][{j}][0]的尺寸为{atten_decoder[i][j][0].shape}')
                    
                    # 上采样（有标记）
                    atten_decoder[i][j][1] = self.up_sampling(atten_decoder[i][j][0], indices[-j - 1])
                    # print(f'解码器中，经过up_sampling后，atten_decoder[{i}][{j}][1]的尺寸为{atten_decoder[i][j][1].shape}')
                    
                    # 上采样（双线性插值）
                    # atten_decoder[i][j][1] = F.interpolate(atten_encoder[i][j][0], size=atten_encoder[i][3-j][2].size()[2:], mode='bilinear',align_corners=True)
                    # print(f'解码器中，经过F.interpolate后，atten_decoder[{i}][{j}][1]的尺寸为{atten_decoder[i][j][1].shape}')
                    
                    atten_decoder[i][j][2] = atten_decoder[i][j][1] + atten_encoder[i][-j - 2][2]
                    
                elif j < 4:
                    atten_decoder[i][j][0] = self.decoder_block[i][j](atten_decoder[i][j - 1][2])
                    # print(f'解码器中，经过decoder_block后，atten_decoder[{i}][{j}][0]的尺寸为{atten_decoder[i][j][0].shape}')
                    ...
                    # 上采样（双线性插值）
                    atten_decoder[i][j][1] = F.interpolate(atten_decoder[i][j][0], size=atten_encoder[i][3-j][2].size()[2:], mode='bilinear',align_corners=True)
                    # print(f'解码器中，经过F.interpolate后，atten_decoder[{i}][{j}][1]的尺寸为{atten_decoder[i][j][1].shape}')
                    ...
                    atten_decoder[i][j][2] = self.decoder_output[i](atten_decoder[i][j][1])
            
        # define task prediction layers
        
        t1_pred = F.log_softmax(self.pred_task1(atten_decoder[0][-1][-1]), dim=1)        # 模型最后输出不用 Kanconv 时用的代码
        t2_pred = self.pred_task2(atten_decoder[1][-1][-1])        # 模型最后输出不用 Kanconv 时用的代码
        
        # t1_pred = F.log_softmax(self.kanconv_task1(self.pred_task1(atten_decoder[0][-1][-1])), dim=1)        # 模型最后输出用 Kanconv 时的代码
        # t2_pred = self.kanconv_task2(self.pred_task2(atten_decoder[1][-1][-1]))        # 模型最后输出用 Kanconv 时的代码
...
optimizer = torch.optim.Adam(paraments, lr=1e-3)        # 使用 imtl 算法时，需要将 KVMSANet_MTAN.parameters() 参数调整为 paraments
